...
function getUid($ScannerPort){
	global $Raspb1IP, $Raspb1Port, $conPi1;
    // read chipID
    $UID = null;
...
function exitAttemp(){
	global $Raspb1IP, $Raspb1Port, $conPi1;
	 $returnValue = $conPi1->readRFID_Byte(1, 8, 0, 0, "FF:FF:FF:FF:FF:FF");
...
function lockChip(){
	global $Raspb1IP, $Raspb1Port, $conPi1;
        // set Bool exitPermitted to false;
         $result = $conPi1->writeRFID_Byte(0, 8, 0, 0, '0', 'FF:FF:FF:FF:FF:FF');
         return $result;
}
...
function unlockChip(){
        ...
        // set exitPermitted to false;
         $result = $conPi1->writeRFID_Byte(0, 8, 0, 0, '1', 'FF:FF:FF:FF:FF:FF');
         return $result;
}
...
function checkSuccessful(){
	global $Raspb1IP, $Raspb1Port, $conPi1;
    //long beep
    $conPi1->setBeeper(true);
    $conPi1->setLED9(true);
	usleep(350000);
	$conPi1->setBeeper(false);
	usleep(10000);
    //short beep
	$conPi1->setBeeper(true);
	usleep(250000);
	$conPi1->setBeeper(false);
	 $conPi1->setLED9(false);
	usleep(10000);
}
